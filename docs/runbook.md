...
**Failure mode:**
- *Client validation fails* — form displays inline field errors; no network request is made.
- *Network error* — catch block in `handleSubmit` shows a top-banner error; form data is preserved so the user can retry.
- *Server returns `{ success: false }`* — banner displays the server's `message` string (or a generic fallback). The form remains editable.
- *Server returns a non-2xx status* — same banner path as above via the `!response.ok` check.
